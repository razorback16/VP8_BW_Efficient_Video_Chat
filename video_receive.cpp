...
int main(int argc, char **argv) {
  ...
  fd = open_listenfd(15213);
  printf("waiting for request\n");
  decoderfd = accept(fd, (SA *)&clientaddr, (socklen_t *)&clientlen);
  printf("request received\n");
  reader = vpx_video_reader_open_c(decoderfd);
  if (!reader)
  ...
  //  die("Failed to open %s for writing.", argv[1]);
  ...
  while (vpx_video_reader_read_frame(reader)) {
    ...
    while ((img = vpx_codec_get_frame(&codec, &iter)) != NULL) {
      ...
      YUV420_To_IplImage(img, rgbImage);
      //cvtColor( img->, gray_image, CV_BGR2GRAY );
      //cvSaveImage("a.jpg",rgbImage);
       cvShowImage( "result", rgbImage );
       //exit(1);
       if( cvWaitKey( 10 ) >= 0 )
            goto _cleanup_;
      ++frame_cnt;
...
_cleanup_:
  ...
  printf("Play: ffplay -f rawvideo -pix_fmt yuv420p -video_size %dx%d %s\n",
